...
public:
    SpectatorList() : head(nullptr), nextId(1) {}
    ...
    void loadFromCSV(const char* filename) {
        ...
        clear(); // Clear any existing data before loading new data
        ...
        while (getline(file, line)) {
            ...
            // Robust parsing of numeric part of ID
            if (!id.empty() && id[0] == 'S') {
                try {
                    string numericPart = id.substr(1);
                    // Remove leading zeros
                    size_t pos = numericPart.find_first_not_of('0');
                    if (pos != string::npos) {
                        numericPart = numericPart.substr(pos);
                    } else {
                        numericPart = "0";
                    }
        ...
        file.close();
        nextId = maxIdNum + 1;  // Next new ID after max found
    }
    ...
    void displayActiveSpectators(const string& roundName) const {
        ...
        while (temp) {
            if (temp->active) {
                cout << count++ << ". ID: " << temp->id << ", Name: " << temp->name
                     << " (Supports Player ID: " << temp->supportedPlayer << ")\n";
            }
            temp = temp->next;
        }
    }
...
void displayMenu() {
    cout << "\n--- Tournament Spectator System Menu ---\n";
    cout << "1. Register new spectators\n";
    cout << "2. Display all spectators\n";
    cout << "3. Display active spectators by round\n";
    cout << "4. Update spectators by match results\n";
    cout << "5. Simulate queue management\n";
    cout << "6. Save spectators to file\n";
    cout << "7. Exit\n";
    cout << "Enter your choice: ";
}

void registerSpectators(SpectatorList& registrationList, MatchList& matches) {
    int numSpectators;
    ...
    for (int i = 0; i < numSpectators; i++) {
        string name, supportedTeam, supportedPlayer, category, day;
        ...
        string generatedId = registrationList.registerSpectator(name, wantsLiveStream, supportedPlayer, category, day);
        cout << "Spectator registered successfully! Your Spectator ID is: " << generatedId << "\n";
    }
}

void displayActiveSpectatorsByRound(SpectatorList& registrationList) {
    cout << "Enter round name (e.g., Round 1, Knockout Round 1): ";
    string roundName;
    getline(cin, roundName);
    registrationList.displayActiveSpectators(roundName);
}

void updateSpectatorsByRound(SpectatorList& registrationList, MatchList& matches) {
    cout << "Enter stage (group/knockout): ";
    string stage;
    getline(cin, stage);
    ...
    updateSpectatorsByMatchResults(registrationList, matches, stage, round);
}

void saveSpectatorsToFile(SpectatorList& registrationList) {
...
int main() {
    ...
    while (!exitProgram) {
        ...
        switch(choice) {
            case 1:
                registerSpectators(registrationList, matches);
                saveSpectatorsToFile(registrationList);  // Save after registration
                break;
            case 2:
                registrationList.displaySpectators();
                break;
            case 3:
                displayActiveSpectatorsByRound(registrationList);
                break;
            case 4:
                updateSpectatorsByRound(registrationList, matches);
                break;
            case 5:
                simulateQueueManagement(registrationList);
                break;
            case 6:
                saveSpectatorsToFile(registrationList);
                break;
            case 7:
                saveSpectatorsToFile(registrationList);  // Save on exit as well
